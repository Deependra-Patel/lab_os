...
static void Net_Device_Receive_Thread(ulong_t arg __attribute__ ((unused))) {
    while (1) {
        Disable_Interrupts();
        if (!Is_Net_Device_Receive_Packet_Queue_Empty(&s_receivePacketQueue)) {
            struct Net_Buf *nBuf;
...
void Init_Network_Devices(void) {
    ...
    Start_Kernel_Thread(Net_Device_Receive_Thread, 0, PRIORITY_NORMAL, false,
                        "{NetRecv}");
}
...
int Register_Net_Device(struct Net_Device_Capabilities *caps,
                        ulong_t baseAddr, ulong_t irq, const char *nameBase) {
    int devNumber;
    int rc;
