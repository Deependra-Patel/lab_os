...
Instances.each { |instance| 
  ...
  pids.push Kernel.fork {
    ...
    command = "%s -name '#{name}' -k en-us -no-acpi %s -m 10 #{StdVga} diskc.img" % [ QEMU, nics.join(' ') ]
    if(DebugInstance == instance.to_i) then
...
def mac(str) 
  (0..5).map { |i| "%02x" % str[i] }.join(':')
end
def ip(str) 
  (0..3).map { |i| "%u" % str[i] }.join('.')
end
def ipp(proto) 
  case proto
  when 17: "UDP"
...
def dump_message(interface, contents)
  ...
  if ether_size == 0x0800 then 
    ...
    puts "%s: %s->%s (%d) %s (overall size: %d)" % [ interface, mac(source_mac), mac(dest_mac), ether_size, (0..payload.length).map { |i| "%02x" % payload[i] }.join(' '), contents.length ]
  end
end
