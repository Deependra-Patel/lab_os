 ...
 */
static union IDT_Descriptor s_IDT[4][NUM_IDT_ENTRIES];
 ...
 */
void Init_IDT(int cpuID) {
    int i;
    ushort_t limitAndBase[3];
    // XXXX only have 4 IDT entries right now
    KASSERT0(cpuID < 4, "Only supports up to 4 cores right now");
    ulong_t idtBaseAddr = (ulong_t) s_IDT[cpuID];
    ...
    for (i = 0, addr = tableBaseAddr; i < NUM_IDT_ENTRIES; ++i) {
        ...
        Init_Interrupt_Gate(&s_IDT[cpuID][i], addr, dpl);
