...
int Get_FS_Buffer(struct FS_Buffer_Cache *cache, ulong_t fsBlockNum,
    ...
    KASSERT0(cache != NULL, "Null FS_Buffer_Cache passed to Get_FS_Buffer.");
    KASSERT0(pBuf != NULL,
...
void Modify_FS_Buffer(struct FS_Buffer_Cache *cache, struct FS_Buffer *buf) {
    KASSERT0(cache != NULL, "Null FS_Buffer_Cache passed to Modify_FS_Buffer.");        /* for warning appeasement */
